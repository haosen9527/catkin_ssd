...
public:
   ...
   *    tensor of block.
   * */
  mobileNet* convBlock(Scope scope,Input inputs,int filters,float alpha,std::vector<int> kernel = {3,3},std::vector<int> strides ={1,1});
  /*
   ...
   *    tensor of block.
   * */
  mobileNet* depthwiseConvBlock(Scope scpoe,Input inputs,int pointwise_conv_filters,float alpha,int depth_multiplier=1,std::vector<int> strides = {1,1},int block_id =1);

  mobileNet* relu6(Scope scope, Input x, std::string name);

  mobileNet* BatchNorm(Scope scope,Input inputs);

  mobileNet* network(Scope scope, float alpha, int depth_multiplier, float dropout, bool include_top, std::string weights, int classes, std::string pooling);

public:
//  tensorflow::Scope scpoe;
